...
Backorder = Shortage['Backorder']
Lost_sales = Shortage['Lost sales']
Backorder_percentage = Shortage['Backorder percentage']
...
eg1.setObjective(
    ...
    + quicksum(Holding_cost[i] * Yp[i][t] * d[i][t] for i in ProductID for t in MonthID)
    + quicksum(g[t]*2750 for t in MonthID)
    + quicksum(Lost_sales[i] * Yn[i][t] * (1-d[i][t]) for t in MonthID for i in ProductID), GRB.MINIMIZE)
