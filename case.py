...
Demand_df = pd.read_excel('OR109-1_case01_data.xlsx', sheet_name='Demand')
# Demand_list = []
# print(Demand_df)
# for i in Demand_df.index:
#     for j in MonthID:
#         print(Demand_df.iat[i, j + 1])
#     print('-----')
# for i in Demand_df.index:
# 	Demand_list.append(list(Demand_df.loc[i][1:]))

# 產品、月份、運送方法編號 i k t
ProductID = range(len(Demand_df['Product']))  # 0-9
...
for i in ProductID:
    ...
    for k in ShippingID:
        x[i].append([])
        for t in MonthID:
            x[i][k].append(p1.addVar(lb=0, vtype=GRB.CONTINUOUS, name="x" + str(i + 1) + str(k + 1) + str(t + 3)))
            # x(1-10),(1-3),(3-8)
#Z binary 變數
z = []
for k in ShippingID:
    z.append([])
    for t in MonthID:
        z[k].append(p1.addVar(lb = 0, vtype = GRB.BINARY, name = "z" + str(k+1) + str(t + 3))) # z(1-3),(3-8)
...
for i in ProductID:
    y.append([])
    for t in MonthID:
        y[i].append(p1.addVar(lb = 0, vtype = GRB.CONTINUOUS, name = "y" + str(i+1) + str(t + 3))) # y(1-10),(3-8)
...
p1.setObjective(purchasing + shipping_f + shipping_v + holding, GRB.MINIMIZE)
