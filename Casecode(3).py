...
for i in ProductID:
	...
	Yn.append([])
	for t in MonthID:
		Yp.append(eg1.addVar(lb = 0, vtype = GRB.CONTINUOUS, name = "y+" + str(i+1) + str(t+3)))
		Yn.append(eg1.addVar(lb = 0, vtype = GRB.CONTINUOUS, name = "y-" + str(i+1) + str(t+3)))
...
eg1.addConstrs(g[t] <= quicksum(Cubic_meter[i] * x[i][2][t] for i in ProductID)/30 + 0.99999999999999 for t in MonthID)
