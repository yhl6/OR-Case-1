...
for i in ProductID:
	y.append([])
	for t in MonthID:
		y[i].append(eg1.addVar(lb = 0, vtype = GRB.INTEGER, name = "y" + str(i+1) + str(t+3)))
...
for t in MonthID:
	...
	for i in ProductID:
		sum += Demandlist[i][t]
		TDemand[t].append(sum)
print(TDemand)

for k in Shipping_method:
	for t in MonthID:
		eg1.addConstr(quicksum(x[i][k][t] for i in ProductID) <= quicksum(TDemand[t][i] * z[k][t] for i in ProductID),'Z constraint')
